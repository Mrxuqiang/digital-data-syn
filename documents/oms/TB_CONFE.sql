/*
================================================================================
表结构代码:TB_CONFE
表结构名称:供应商合同变更期间费用表
表结构目的:
================================================================================
*/

drop sequence SEQ_TB_CONFE;
drop index AK_TB_CONFE;
drop table TB_CONFE;
create table TB_CONFE  (
   CONFE_ID             INTEGER                         not null,  /*合同变更期间费用ID  */
   CONFE001             INTEGER                         not null,  /*合同变更ID          */
   CONFE002             INTEGER                         not null,  /*营运组织ID          */
   CONFE003             INTEGER                         not null,  /*费用ID              */
   CONFE004             VARCHAR2(1)                     not null,  /*费用周期            */
   CONFE005             VARCHAR2(1)                     not null,  /*费用产生时间        */
   CONFE006             DATE,                                      /*费用日期            */
   CONFE007             VARCHAR2(1)                     not null,  /*基准营运组织        */
   CONFE008             VARCHAR2(1)                     not null,  /*基准费用            */
   CONFE009             NUMBER(12,2),                              /*费用金额            */
   CONFE010             NUMBER(12,2),                              /*基准费用比率(%)     */
   CONFE011             NUMBER(12,2),                              /*费用计提比例(%)     */
   CONFE012             DATE,                                      /*开始日期            */
   CONFE013             DATE,                                      /*截止日期            */
   CONFE014             NUMBER(12,2),                              /*费用下限            */
   CONFE015             NUMBER(12,2),                              /*费用上限            */
   CREATE_USER          VARCHAR2(12),                              /*建立人员            */
   USER_GROUP           VARCHAR2(12),                              /*建立人员部门        */                               
   CREATE_DATE          DATE,                                      /*建立日期            */
   MODIFIER             VARCHAR2(12),                              /*修改人员            */
   MODI_DATE            DATE,                                      /*修改日期            */
   FLAG                 NUMBER(1),                                 /*资料状态            */
   constraint PK_TB_CONFE primary key (CONFE_ID)
);
create unique index AK_TB_CONFE on TB_CONFE (CONFE001,CONFE002,CONFE003,CONFE004,CONFE005);
create sequence SEQ_TB_CONFE minvalue 1 maxvalue 9999999999999999999999999999 start with 1 increment by 1 nocache;
grant select on TB_CONFE to public;
grant index  on TB_CONFE to public;
grant update on TB_CONFE to public; 
grant delete on TB_CONFE to public;  
grant insert on TB_CONFE to public; 
grant select on SEQ_TB_CONFE to public;   